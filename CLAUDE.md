...
- `dreamerv3/decode_position.py` — linear classification decoders to predict (x,y) from deter/stoch. Two modes: `standard` (single-repr decoding with CV) and `layers` (per-layer comparison boxplot). Includes `plot_probmap_on_world` (P(pos) heatmap on world map) and `plot_occupancy_vs_error` (occupancy hotspots + Manhattan error scatter). Occupancy figures (`occupancy_vs_error_*.svg`, `occupancy_fullset_*.svg`) show a **pRNN-style coverage** metric in the title via `calculate_coverage` (port of pRNN `trajectoryAnalysis.calculateCoverage`): `coverage = 1 − max|p_occ − p_uniform|` over the *walkable* tiles (grass/path/sand, reconstructed from the world seed by `plot_trajectories._extract_walkable_mask`; falls back to whole-grid uniform when `env_seed` is unrecorded — same seed plumbing as the world-map background), plus the KL-to-uniform variant and literal visited-fraction of reachable tiles. `--save_model` saves fitted decoders for dream_decode. Metric: mean Manhattan distance (tiles)
...
- `embodied/envs/crafter.py` — Crafter wrapper. `fixed_seed=True` resets `_episode=0` before each reset so same world. `fixed_layout=True` is the **intermediate** mode between `fixed_seed` True/False: identical terrain *shell* (grass/water/sand/mountain/path/lava boundaries) every episode, but resources (tree/coal/iron/diamond) and mobs (cow/zombie/skeleton) are resampled per episode. Implemented by monkeypatching `crafter.worldgen.generate_world` (`_install_fixed_layout_worldgen`) to split the single per-episode RNG into a fixed `_layout_seed` (drives the OpenSimplex terrain noise → water/stone/sand/grass shell) and a per-episode `_content_seed` (drives the `world.random.uniform` draws that decide resource realization: which mountain tiles are coal/iron/diamond vs stone, which grass tiles are trees). The `_layout_seed` is derived by `fixed_layout_simplex_seed(env_seed)` = `RandomState(hash((env_seed,1)) % (2**31-1)).randint(0, 2**31-1)`, which **reproduces the base seed's `fixed_seed` (episode-1) terrain shell** — so a fixed_layout run shares the *identical* water/stone/sand skeleton as the non-fixed-layout net on the same seed (directly comparable; agent never appears on water), while resources+mobs resample per episode. (Prior to 2026-09-15 the layout seed was `hash((env_seed,-1))` used *directly* as the OpenSimplex seed, which produced a **different** world than the base seed — runs from before that fix, e.g. `masked_k5_invert_reg_wrld45_fixedlayout_normrms_gateFalse`, are NOT world-45-comparable and need retraining.) Note: because trees occupy grass tiles and are non-walkable, resource resampling *does* shift the walkable set slightly per episode (grass↔tree); the water/stone/sand skeleton stays frozen. `content_seed` int-only `hash()` tuples (process-stable). `fixed_layout_simplex_seed` is shared by the env and `plot_trajectories._reconstruct_crafter_env` so background/mask never drift from the generated world. Takes precedence over `fixed_seed` when both set. `island_border=True` turns the square area into an irregular water-bordered **island/blob** (vs the default boxy world): `_island_set_material` (a copy of crafter's `_set_material`) adds a radial "coast" term centered on the world center that raises the `water` field toward the edges, perturbed by simplex (`island_roughness`, default 0.16) so the coastline is a blob not a circle; `island_fill` (default 0.72 = fraction of the inscribed radius kept as land) sets island size. Interior is untouched stock terrain; crafter's own `0.25<water≤0.35→sand` rule paints beaches, and `_set_object` keeps mobs off the ocean for free. Shares the unified `_install_worldgen_patch` (the old `_install_fixed_layout_worldgen` name is now an alias) driven by a `world._island` params dict stamped in `step()`; composes with `fixed_layout`. Plot backgrounds render the blob when `metadata['island']` is present (recorded by eval_trajectory). **fixed_layout plot reconstruction**: eval_trajectory now records `fixed_layout: True` in metadata, and `plot_trajectories._reconstruct_crafter_env` (shared by `_render_crafter_world` + `_extract_walkable_mask`) rebuilds the true terrain shell by setting `world._layout_seed = hash((env_seed, -1)) % (2**31-1)` before reset — WITHOUT this, stock worldgen draws water in the wrong tiles and occupancy dots/coverage land on "water" (~25% spurious water overlap → 0% once reconstructed). For older runs whose metadata predates this, pass `--fixed_layout` to `plot_trajectories.py` / `decode_position.py` to force the reconstruction (layout seed derives purely from the already-recorded `env_seed`). `random_spawn=True` relocates player to random walkable tile each episode. `egocentric_view=N` (odd int, e.g. 7) renders N×N egocentric view centered on player facing direction; inventory bar is copied from the standard render into the bottom rows. Also exposes: `log/player_facing_x`, `log/player_facing_y` (facing direction as ±1/0 ints), `log/achievement_*` (per-achievement binary), `log/reward` (raw crafter reward). Writes `stats.jsonl` to logdir if configured
